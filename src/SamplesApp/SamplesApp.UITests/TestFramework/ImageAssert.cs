...
namespace SamplesApp.UITests.TestFramework
{
	public static class ImageAssert
	{
		...
		{
			AreAlmostEqualImpl(expected, expectedRect, actual, actualRect, PixelTolerance.None, line);
		}
		#endregion
		...
		public static void AreAlmostEqual(FileInfo expected, FileInfo actual, IAppRect rect, int permittedPixelError, [CallerLineNumber] int line = 0)
			=> AreAlmostEqualImpl(expected, rect.ToRectangle(), actual, rect.ToRectangle(), new PixelTolerance((byte)permittedPixelError, ColorToleranceKind.Cumulative, default), line);
		...
		public static void AreAlmostEqual(FileInfo expected, FileInfo actual, Rectangle? rect = null, int permittedPixelError = 5, [CallerLineNumber] int line = 0)
			=> AreAlmostEqualImpl(expected, rect ?? FirstQuadrant, actual, rect ?? FirstQuadrant, new PixelTolerance((byte)permittedPixelError, ColorToleranceKind.Cumulative, default), line);
		...
		public static void AreAlmostEqual(FileInfo expected, IAppRect expectedRect, FileInfo actual, IAppRect actualRect, int permittedPixelError, [CallerLineNumber] int line = 0)
			=> AreAlmostEqualImpl(expected, expectedRect.ToRectangle(), actual, actualRect.ToRectangle(), new PixelTolerance((byte)permittedPixelError, ColorToleranceKind.Cumulative, default), line);
		...
		public static void AreAlmostEqual(FileInfo expected, Rectangle expectedRect, FileInfo actual, Rectangle actualRect, int permittedPixelError, [CallerLineNumber] int line = 0)
			=> AreAlmostEqualImpl(expected, expectedRect, actual, actualRect, new PixelTolerance((byte)permittedPixelError, ColorToleranceKind.Cumulative, default), line);

		/// <summary>
		/// Asserts that two screenshots are equal to each other inside the area of the nominated rect to within the given pixel error
		/// (ie, the A, R, G, or B values cumulatively differ by more than the permitted error for any pixel).
		/// </summary>
		public static void AreAlmostEqual(FileInfo expected, Rectangle expectedRect, FileInfo actual, Rectangle actualRect, PixelTolerance tolerance, [CallerLineNumber] int line = 0)
			=> AreAlmostEqualImpl(expected, expectedRect, actual, actualRect, tolerance, line);

		private static void AreAlmostEqualImpl(FileInfo expected, Rectangle expectedRect, FileInfo actual, Rectangle actualRect, PixelTolerance tolerance, int line)
		{
			...
			using (var actualBitmap = new Bitmap(actual.FullName))
			{
				...
				if (!Validate(actualBitmap, expectedPixels, report))
				{
		...
		private static bool Validate(Bitmap actualBitmap, ExpectedPixels expectation, StringBuilder report)
		{
			...
			bool ValidatePixels(int offsetX, int offsetY)
			{
				...
				for (var col = 0; col < expectation.Values.GetLength(1); col++)
				{
					...
					var actualX = expectation.Location.X + col + offsetX;
					var actualY = expectation.Location.Y + lin + offsetY;
